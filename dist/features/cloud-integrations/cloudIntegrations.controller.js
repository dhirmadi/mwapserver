...
import { createCloudProviderIntegrationSchema, updateCloudProviderIntegrationSchema, CloudProviderIntegrationErrorCodes } from '../../schemas/cloudProviderIntegration.schema.js';
import { getDB } from '../../config/db.js';
import { ObjectId } from 'mongodb';
const cloudIntegrationsService = new CloudIntegrationsService();
// Helper function to check if user is tenant owner
async function isTenantOwner(tenantId, userId) {
    try {
...
export async function getTenantIntegrations(req, res) {
...
export async function getTenantIntegrationById(req, res) {
    const user = getUserFromToken(req);
...
export async function createTenantIntegration(req, res) {
    try {
        const user = getUserFromToken(req);
        const { tenantId } = req.params;
        // Check if user is tenant owner
...
export async function updateTenantIntegration(req, res) {
    try {
        const user = getUserFromToken(req);
        const { tenantId, integrationId } = req.params;
        // Check if user is tenant owner
...
export async function deleteTenantIntegration(req, res) {
    const user = getUserFromToken(req);
    const { tenantId, integrationId } = req.params;
    // Check if user is tenant owner
    const isOwner = await isTenantOwner(tenantId, user.sub);

...
import { ProjectErrorCodes } from '../../schemas/project.schema.js';
export class ProjectsService {
    constructor() {
        this.collection = getDB().collection('projects');
    }
    ...
    async update(id, data, userId) {
        ...
        const result = await this.collection.findOneAndUpdate({ _id: project._id }, { $set: updates }, { returnDocument: 'after' });
        if (!result) {
    ...
    async delete(id, userId, isSuperAdmin = false) {
        const project = await this.findById(id, userId);
    ...
    async addMember(projectId, memberData, userId) {
        ...
        // Add new member
        await this.collection.updateOne({ _id: project._id }, {
    ...
    async updateMember(projectId, memberId, data, userId) {
        const project = await this.findById(projectId, userId);
        ...
        const isLastOwner = project.members[memberIndex].role === 'OWNER' &&
            data.role !== 'OWNER' &&

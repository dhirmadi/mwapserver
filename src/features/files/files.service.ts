...
import { File, FileQuery, FileErrorCodes } from '../../schemas/file.schema.js';
...
export class FilesService {
   ...
   */
  async listFiles(projectId: string, query: FileQuery, userId: string): Promise<File[]> {
    try {
      // Get project details
      ...
      const files = await this.fetchFilesFromProvider(
        cloudProvider.slug,
        cloudIntegration.accessToken,
        fullPath,
        query
      );
      ...
      throw new ApiError(
        'Failed to list files from cloud provider',
        500,
        FileErrorCodes.CLOUD_PROVIDER_ERROR
      );
    }
  }
   ...
   */
  private async fetchFilesFromProvider(
    providerSlug: string,
    accessToken: string,
    folderPath: string,
    query: FileQuery
    ...
    switch (providerSlug) {
      ...
      default:
        throw new ApiError(
          `Unsupported cloud provider: ${providerSlug}`,
          400,
          FileErrorCodes.CLOUD_PROVIDER_ERROR
        );
    }
   ...
   */
  private async fetchFilesFromGDrive(
    accessToken: string,
    folderPath: string,
    query: FileQuery
  ): Promise<File[]> {
    try {
      ...
      throw new ApiError(
        ...
        500,
        FileErrorCodes.INTEGRATION_ERROR
      );
    }
  }
   ...
   */
  private async findGDriveFolderId(accessToken: string, folderPath: string): Promise<string | null> {
   ...
   */
  private async fetchFilesFromDropbox(
    accessToken: string,
    folderPath: string,
    query: FileQuery
  ): Promise<File[]> {
    try {
      ...
      throw new ApiError(
        'Failed to fetch files from Dropbox',
        500,
        FileErrorCodes.INTEGRATION_ERROR
      );
    }
   ...
   */
  private async fetchFilesFromOneDrive(
    accessToken: string,
    folderPath: string,
    query: FileQuery
  ): Promise<File[]> {
    try {
      ...
      throw new ApiError(
        'Failed to fetch files from OneDrive',
        500,
        FileErrorCodes.INTEGRATION_ERROR
      );
    }
